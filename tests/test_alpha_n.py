...
"""
    ...
    def _fake_read_sag4n_output(input_data):  # noqa: ARG001
        empty = ak.Array(
            {
                k: []
                for k in [
        ...
        return {"evts": empty, "integral_yield": 0.0}

    monkeypatch.setattr(alpha_n, "run_sag4n", _fake_run_sag4n)
    monkeypatch.setattr(alpha_n, "read_sag4n_output", _fake_read_sag4n_output)
    monkeypatch.setattr(
        alpha_n,
...
def test_call_to_sag4n(tmp_path):
    ...
    assert set(result["evts"].fields) == expected_fields
    assert len(result["evts"]["evtid"]) > 0
    assert isinstance(result["integral_yield"], float)
    assert result["integral_yield"] >= 0
    assert all(p in {"neutron", "gamma"} for p in result["evts"]["particle"].to_list())
...
### save sag4n output to lh5


def test_save_sag4n_output_to_lh5_roundtrip(tmp_path):
    output_file = tmp_path / "test_output.lh5"

    prepared_output = ak.Array(
        {
            "px": np.array([1.0, 0.0, 0.0]),
            "py": np.array([0.0, 1.0, 0.0]),
            "pz": np.array([0.0, 0.0, 1.0]),
            "ekin": np.array([10.0, 20.0, 30.0]),
            "time": np.array([0.0, 0.0, 0.0]),
            "g4_pid": np.array([2112, 22, 2112], dtype=int),
            "n_part": np.array([2, 0, 1], dtype=int),
        }
    )
    output_data = {
        "prepared_output": prepared_output,
        "integral_yield": 1.23e-5,
        "n_valid_events": 2,
        "n_simulated_events": 10000,
    }

    alpha_n.save_sag4n_output_to_lh5(output_data, output_file)

    assert output_file.exists()

    kin = lh5.read("vtx/kin", str(output_file))
    assert set(kin.keys()) == {"px", "py", "pz", "ekin", "time", "g4_pid", "n_part"}
    np.testing.assert_array_almost_equal(kin["ekin"].nda, [10.0, 20.0, 30.0])
    assert kin["ekin"].attrs.get("units") == "MeV"
    assert kin["time"].attrs.get("units") == "ns"
    assert kin["g4_pid"].nda.dtype == np.int64
    assert kin["n_part"].nda.dtype == np.int64

    misc = lh5.read("misc", str(output_file))
    assert float(misc["integral_yield"].value) == pytest.approx(1.23e-5)
    assert int(misc["n_valid_events"].value) == 2
    assert int(misc["n_simulated_events"].value) == 10000


### container runtime


def test_detect_container_runtime_raises_for_unsupported_runtime():
    with pytest.raises(ValueError, match="Unsupported container runtime"):
        alpha_n._detect_container_runtime({"container_runtime": "singularity"})
